...
interface UploadViewProps {
  onDocumentProcessed: (newDoc: DocumentItem) => void;
  onNavigateToChat: (doc: DocumentItem) => void;
  onNavigateToSplit: (doc: DocumentItem) => void;
}
...
export const UploadView: React.FC<UploadViewProps> = ({
  ...
  const startPipeline = (fileName: string, fileSize: string, fileType: 'PDF' | 'DOCX' | 'TXT') => {
    ...
    // Simulate RAG pipeline step by step
    setTimeout(() => {
      setPipelineSteps((prev) =>
        prev.map((s, idx) => (idx === 0 ? { ...s, status: 'completed' } : idx === 1 ? { ...s, status: 'in_progress' } : s))
      );
    }, 900);

    setTimeout(() => {
      setPipelineSteps((prev) =>
        prev.map((s, idx) => (idx <= 1 ? { ...s, status: 'completed' } : idx === 2 ? { ...s, status: 'in_progress' } : s))
      );
    }, 1800);

    setTimeout(() => {
      setPipelineSteps((prev) =>
        prev.map((s, idx) => (idx <= 2 ? { ...s, status: 'completed' } : idx === 3 ? { ...s, status: 'in_progress' } : s))
      );
    }, 2800);

    setTimeout(() => {
      setPipelineSteps((prev) =>
        prev.map((s, idx) => (idx <= 3 ? { ...s, status: 'completed' } : idx === 4 ? { ...s, status: 'in_progress' } : s))
      );
    }, 3900);
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#fcfcfc] overflow-y-auto items-center justify-center p-8">
      <div className="w-full max-w-2xl">
        ...
        {!selectedFile ? (
          <div
            ...
            className={`border-2 border-dashed rounded-2xl p-12 text-center transition-all bg-white cursor-pointer group ${
              isDragging
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        ...
                        {isInProgress && (
                          <span className="text-[11px] font-mono text-blue-600 font-medium animate-pulse">
                            Processing...
                          </span>
                        )}
                        {isCompleted && (

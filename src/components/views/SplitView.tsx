...
import { DocumentItem, ChatMessage, DocumentSourceCitation } from '../../types';
...
interface SplitViewProps {
  ...
  isLoading?: boolean;
  activeCitation?: DocumentSourceCitation | null;
  onCitationSelect: (citation: DocumentSourceCitation) => void;
}
...
export const SplitView: React.FC<SplitViewProps> = ({
  ...
  const [currentPage, setCurrentPage] = useState<number>(4); // Default to Page 4 where first citation is
  ...
  // When active citation changes, jump to that page!
  useEffect(() => {
    if (activeCitation) {
  ...
  return (
    <div className="flex-1 flex h-full overflow-hidden bg-neutral-100">
          ...
          <div className="flex items-center gap-1 text-neutral-500">
            ...
            <button
              ...
              className={`p-1.5 rounded transition-colors cursor-pointer ${
                highlightActive ? 'bg-amber-100 text-amber-800 font-semibold' : 'hover:bg-neutral-100 text-neutral-500'
              }`}
              ...
              <button
                ...
                className={`px-2 py-0.5 rounded text-[11px] font-medium transition-colors cursor-pointer ${
                  currentPage === p
                    ...
                    : 'bg-white text-neutral-700 hover:bg-amber-100 border border-amber-200/80'
                }`}
              >
                Page {p}
              </button>
                ...
                {pageData.sections.map((section, sIdx) => {
                  return (
                    <div key={sIdx} className="space-y-3">
                      {section.heading && (
                        <h3 className="font-sans font-bold text-xs uppercase tracking-wide text-neutral-800 mt-4 mb-2">
                          {section.heading}
                        </h3>
                      )}
                      ...
                      {section.paragraphs.map((pText, pIdx) => {
                        const isCitationHighlighted =
                          highlightActive &&
                          (activeCitation?.page === currentPage ||
                            (currentPage === 4 && pText.includes('Combining dense vector embeddings')) ||
                            (currentPage === 8 && pText.includes('optimal semantic chunk overlap')) ||
                            (currentPage === 12 && pText.includes('Cross-encoder re-ranking')));
                        ...
                        return (
                          <p key={pIdx} className="leading-relaxed select-text">
                            {pText}
                          </p>
                        );
                      })}

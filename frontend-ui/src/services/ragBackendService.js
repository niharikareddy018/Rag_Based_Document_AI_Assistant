...
export async function queryGatewayAsk(userPrompt, authToken = DEFAULT_AUTH_TOKEN, activeDocumentName = "Microservice_Firewall_Rules.pdf", documentText) {
  try {
  ...
  if (promptLower.includes("firewall") || promptLower.includes("microservice") || promptLower.includes("rule") || promptLower.includes("port")) {
    return {
      answerText: "Microservice firewall configurations require zero-trust perimeter isolation. External ingress is strictly locked to port 8086 via the Spring Boot gateway, internal RabbitMQ message exchanges reside on port 5672, and PostgreSQL pgvector database queries on port 5432 must be restricted to internal container bridges with authenticated bearer tokens.",
  ...
  if (promptLower.includes("rabbitmq") || promptLower.includes("queue") || promptLower.includes("chunk") || promptLower.includes("worker")) {
    return {
      answerText: 'Document ingestion utilizes RabbitMQ with the durable queue "document-task-queue". When an upload arrives at GatewayController, the raw text payload is pushed as "documentId||filename||rawTextContent". The ChunkingWorker listener slices the text into 50-character semantic chunks, passes each window to nomic-embed-text on Ollama, and commits the resulting 768-dimension vectors to pgvector.',
      documentName: "Microservice_Firewall_Rules.pdf",
      findings: [
      ...
      conclusion: "Asynchronous queue prevents HTTP timeouts during heavy document processing.",
      citations: [
        {
          id: "cite-rmq-1",
          page: 3,
  ...
  if (promptLower.includes("ollama") || promptLower.includes("llama") || promptLower.includes("tinyllama") || promptLower.includes("model") || promptLower.includes("nomic")) {
    return {
      ...
      conclusion: "Local Llama 3 execution guarantees zero data leakage and 100% privacy for company documents.",
      citations: [
        {
          id: "cite-ollama-1",
          page: 1,
          ...
          previewSnippet: "Ollama AI service container hosts nomic-embed-text for vectorization and Llama 3 for deterministic corporate question answering.",
          highlightText: "Ollama AI service container hosts nomic-embed-text for vectorization and Llama 3 for deterministic corporate question answering.",
          relevanceScore: 96.1,
...
export async function uploadDocumentToGateway(fileName, rawContent, authToken = DEFAULT_AUTH_TOKEN) {
  try {

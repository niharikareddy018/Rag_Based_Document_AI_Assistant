...
export const AuthModal = ({
  ...
  const handleEmailAuth = (e) => {
    ...
    setTimeout(() => {
      ...
      if (!displayName) {
        ...
        displayName = prefix.split(/[._-]/).map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(" ");
      }
      const validatedUser = {
        id: `usr_${Date.now()}`,
  ...
  const handleGoogleSignIn = () => {
    ...
    setTimeout(() => {
      ...
      const derivedName = prefix.split(/[._-]/).map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(" ");
      const validatedUser = {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in select-none">
      <div
  ...
  >
    ...
    /* Validation Error Alert */
          ...
          {validationError && <div className="flex items-start gap-2.5 p-3 rounded-xl bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900/60 text-red-700 dark:text-red-400 text-xs animate-in fade-in">

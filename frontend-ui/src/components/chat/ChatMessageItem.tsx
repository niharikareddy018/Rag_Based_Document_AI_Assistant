...
/**
 * Message Item with Document Attachment Card and Conversational Flow
 * Supports Dark Mode
 */
export const ChatMessageItem: React.FC<ChatMessageItemProps> = ({
    ...
    return (
      <div className="flex justify-end mb-6">
        <div className="max-w-2xl bg-[#f4f4f4] dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100 px-4 py-2.5 rounded-3xl text-sm leading-relaxed shadow-2xs font-normal border border-transparent dark:border-neutral-700/60 transition-colors">
          {message.attachedDoc && (
            <div className="flex items-center gap-2 px-3 py-1.5 mb-2 rounded-xl bg-white dark:bg-neutral-750 border border-neutral-200 dark:border-neutral-700 text-xs text-neutral-800 dark:text-neutral-200">
  ...
  // Assistant message (clean left-aligned unique typography)
  return (
    <div className="flex justify-start mb-8 group">
      <div className="flex gap-4 max-w-3xl w-full items-start">
        ...
        <div className="flex-1 min-w-0 space-y-3">
          ...
          {message.attachedDoc && (
            <div className="flex items-center gap-3 p-3 rounded-2xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-200/90 dark:border-neutral-800 text-neutral-900 dark:text-neutral-100 shadow-2xs">
          ...
          {cleanFindingsList && cleanFindingsList.length > 0 && (
            <div className="pt-1 space-y-1.5">
              <div className="text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
                Key Points
              </div>
              <ul className="space-y-1.5 pl-1">
                {cleanFindingsList.map((f, idx) => (
                  <li key={idx} className="flex items-start gap-2 text-sm text-neutral-800 dark:text-neutral-200 leading-relaxed">
          ...
          <div className="flex items-center gap-1 pt-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button
              ...
              className={`p-1.5 rounded-md hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors cursor-pointer ${
                feedback === 'up' ? 'text-emerald-600' : 'text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200'
              }`}
            >
            ...
            <button
              ...
              className={`p-1.5 rounded-md hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors cursor-pointer ${
                feedback === 'down' ? 'text-red-500' : 'text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200'
              }`}
            >
